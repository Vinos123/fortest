...
class AP():
    ...
    def listen_client(self,port=8080):  #一般默认8080
        try:    
            self.ip = self.ap.ifconfig()[0][0]
            self.listenSocket = socket.socket()   #创建套接字
            self.listenSocket.bind(('192.168.4.1', port))   #绑定地址和端口号 地址可以通过network.WLAN(network.AP_IF).ifconfig()返回得到
    ...
    def read(self):
        while 1:
            ...
            if len(self.read_data)>0:
                ...
                self.uart.write(self.read_data.encode())
    ...
    def send_from_uart(self):
        while 1:
            uart_len = self.uart.any()
            if uart_len > 0:
                text = self.uart.read()
                text = text.decode()
                self.send(text)
    ...
    def send_thread(self):
        _thread.start_new_thread(self.send_from_uart,())
